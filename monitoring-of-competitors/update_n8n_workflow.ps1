# N8n Workflow Update via REST API with new session cookie
...
# New code for Check If Company Exists1
$checkIfExistsCode = @'
...
try {
    ...
    if ($formatNode) {
        ...
        $formatNode.parameters.jsCode = $formatForSheetsCode
    } else {
    ...
    if ($checkNode) {
        Write-Host "  Found Check If Company Exists1 - updating code..." -ForegroundColor Yellow
        $checkNode.parameters.jsCode = $checkIfExistsCode
    } else {
        Write-Host "  ERROR: Check If Company Exists1 not found!" -ForegroundColor Red
    }
