...
const data = JSON.parse(fs.readFileSync('C:/Users/izeak/.claude/projects/C--Users-izeak-OneDrive-Work-Oki-toki-Monitoring-of-competitors/c07261c1-a225-44cb-a2ce-90406882e3d3/tool-results/mcp-n8n-flexible-n8n_get_workflow-1766659230155.txt'));
const workflow = JSON.parse(data[0].text);

// Новий код для Extract & Filter URLs
const newExtractCode = `// Extract & Filter URLs - отримуємо companyName/companyUrl з Edit Fields
...
// Новий код для Parse Page Content
const newParseCode = `// Parse Page Content - V3 з розширеним парсингом
...
const parseNode = workflow.nodes.find(n => n.name === 'Parse Page Content');
if (parseNode) {
  parseNode.parameters.jsCode = newParseCode;
  console.log('Updated Parse Page Content');
}

// Зберігаємо
fs.writeFileSync('C:/Users/izeak/OneDrive/Work.Oki-toki/Monitoring of competitors/workflow_final_v4.json', JSON.stringify(workflow, null, 2));
console.log('Saved to workflow_final_v4.json');
