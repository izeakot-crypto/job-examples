...
async function n8nRequest(endpoint: string, options: any = {}): Promise<any> {
...
function findNodeByName(workflow: any, nodeName: string) {
  if (!workflow.nodes) return null;
  return workflow.nodes.find((n: any) => n.name === nodeName || n.id === nodeName);
}

// Helper: Find node by ID in workflow
function findNodeById(workflow: any, nodeId: string) {
  if (!workflow.nodes) return null;
  return workflow.nodes.find((n: any) => n.id === nodeId);
}

// Helper: Clean workflow for PUT request (remove read-only fields)
function cleanWorkflowForUpdate(workflow: any): any {
  ...
  try {
    switch (name) {
      case "n8n_set_api_key": {
        const { apiKey, url } = args as { apiKey: string; url?: string };
        if (url) N8N_URL = url;
        N8N_API_KEY = apiKey;
        return {
          content: [{
      ...
      case "n8n_set_session": {
        const { cookie, url } = args as { cookie: string; url?: string };
        if (url) N8N_URL = url;
      ...
      case "n8n_get_node": {
        const { workflowId, nodeName } = args as { workflowId: string; nodeName: string };
        const workflow = await n8nRequest(`/api/v1/workflows/${workflowId}`);
      ...
      case "n8n_update_node_code": {
        const { workflowId, nodeName, code } = args as {
          workflowId: string;
          nodeName: string;
          code: string;
        };
      ...
      case "n8n_update_node_parameters": {
        const { workflowId, nodeName, parameters } = args as {
          workflowId: string;
          nodeName: string;
      ...
      case "n8n_add_node": {
        const { workflowId, node, connectFrom } = args as {
          workflowId: string;
          node: any;
          connectFrom?: string;
        };
