...
from .runner import run_plan, RunProgress
...
class PlanPoller:
    ...
    def __init__(self) -> None:
        ...
        self._active_plan_ids: set[int] = set()  # для дедупликации
    ...
    def enqueue(self, comp_id: int, plan_id: int) -> bool:
        """Добавляет план в очередь. Возвращает False если уже в очереди."""
        if plan_id in self._active_plan_ids:
            logger.debug("План %d уже в очереди — пропускаем", plan_id)
            return False

        task = PlanTask(comp_id=comp_id, plan_id=plan_id)
        self._active_plan_ids.add(plan_id)
        self._queue.put_nowait(task)
        logger.info("План %d (comp_id=%d) добавлен в очередь (размер: %d)",
                     plan_id, comp_id, self._queue.qsize())
        return True
    ...
    async def _worker_loop(self) -> None:
        ...
        while True:
            try:
                ...
                logger.info("Начинаю обработку плана %d (comp_id=%d)", task.plan_id, task.comp_id)
                ...
                try:
                    ...
                    logger.info("План %d завершён: %s", task.plan_id, result)
                except Exception:
                    logger.exception("Ошибка при обработке плана %d", task.plan_id)
                    self._last_result = {
                        "comp_id": task.comp_id,
                        "plan_id": task.plan_id,
