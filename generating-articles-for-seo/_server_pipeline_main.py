...
# ── Rewrite Idea via Claude ────────────────────────────────────

@app.post("/rewrite-idea")
async def rewrite_idea_endpoint(req: RewriteIdeaRequest):
    """Rewrite an idea using Claude based on user edits + comments."""
    from services.anthropic_client import rewrite_idea
    ...
    # Fetch original idea
    try:
        original_result = client.table("ideas").select("*").eq("id", req.idea_id).single().execute()
        original = original_result.data
    except Exception as e:
        raise HTTPException(status_code=404, detail=f"Idea not found: {e}")
    ...
    try:
        refined = await rewrite_idea(original, edited, req.comments)
    except Exception as e:
        logger.error(f"[Rewrite] Claude failed: {e}")
        raise HTTPException(status_code=500, detail=f"AI rewrite failed: {e}")
    ...
    try:
        client.table("ideas").update({
            ...
            "priority_score": refined.get("priority_score", 75),
        }).eq("id", req.idea_id).execute()
    except Exception as e:
        logger.error(f"[Rewrite] DB update failed: {e}")
