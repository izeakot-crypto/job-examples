...
async def process_call(
    ...
    try:
        ...
        if not has_transcript:
            # Попробовать запросить создание транскрибации у Вадима
            requested = await client.request_transcription(comp_id, session_id)
            if requested:
                logger.info(
                    "session_id=%d — транскрипция отсутствует, запрошено создание. "
                    "Звонок будет обработан после webhook от Вадима.",
                    session_id,
                )
                progress.skipped += 1
                ...
                return CallReport(
                    ...
                    skip_reason=reasons["no_transcript"],
                )
            else:
                # Нет аудиозаписи вообще — скипаем
                reason = reasons["no_transcript"]
                logger.warning("session_id=%d — нет аудио, пропускаем", session_id)
...
async def process_single_call(comp_id: int, plan_id: int, session_id: int) -> dict:
    """Обрабатывает один звонок (webhook от Вадима). Возвращает статус."""
    async with OkiTokiClient(comp_id=comp_id) as client:
        evaluator = Evaluator()
        semaphore = asyncio.Semaphore(1)
        progress = RunProgress(total=1)

        # Определяем язык по анкете
        sheet = await client.get_call_sheet(plan_id, session_id)
        language = detect_language(get_sheet_text(sheet))

        report = await process_call(
            client, evaluator, semaphore, comp_id, plan_id, session_id, progress, language,
        )
